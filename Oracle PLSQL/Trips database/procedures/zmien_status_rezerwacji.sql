create or replace procedure zmien_status_rezerwacji (id_rezerwacji int, status char)
as
...
begin
    ...
    update REZERWACJE r
    set
        r.STATUS = zmien_status_rezerwacji.status
    where NR_REZERWACJI = zmien_status_rezerwacji.id_rezerwacji;

    insert into REZERWACJE_LOG (ID_REZERWACJI, DATA, STATUS)
    values (zmien_status_rezerwacji.id_rezerwacji, sysdate, 'N');
